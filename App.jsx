        ...
        <section className="lg:col-span-2 space-y-4">
          <div className="bg-white rounded-xl shadow overflow-hidden">
            <MapContainer center={center} zoom={5} style={{ height: "560px", width: "100%" }} whenCreated={(map) => (mapRef.current = map)}>
              ...
              {waypoints.map((w, i) => w.place && (
                <Marker key={`wp-${i}`} position={[w.place.lat, w.place.lon]}>
              ...
              <FitBounds
                points={[...(route || []), ...(origin ? [[origin.lat, origin.lon]] : []), ...(dest ? [[dest.lat, dest.lon]] : [])]}
              />
            </MapContainer>
          </div>

          <div className="bg-white p-4 rounded-xl shadow-sm">
            <div className="text-sm font-medium text-gray-700 mb-2">5-day Forecast (destination)</div>
            <div className="flex gap-2 overflow-x-auto">
              {forecast.length === 0 ? (
                <div className="text-xs text-gray-500">No forecast</div>
              ) : (
                forecast.map((f) => (
                  <div key={f.date} className="min-w-[110px] bg-gray-50 p-2 rounded-lg text-center">
                    <div className="text-xs text-gray-500">{f.date}</div>
                    <div className="font-semibold">{f.temp}°C</div>
                    <div className="text-xs capitalize text-gray-600">{f.desc}</div>
                  </div>
                ))
              )}
            </div>
